...
import { 
  ...
  Play, X, Cpu, Search, BarChart3, Lock, MessageSquare
} from "lucide-react";
...
export default function LandingPage() {
  ...
  const CALENDLY_URL = "https://calendly.com/roleflow/discovery"; // Replace with your link
    ...
    <div className="bg-[#030712] text-white min-h-screen font-sans selection:bg-blue-500 overflow-x-hidden">
      ...
      <nav className="px-6 py-8 max-w-7xl mx-auto flex justify-between items-center relative z-50">
        <div className="flex items-center gap-4">
          {/* Logo from GitHub Public Folder */}
      ...
      <section className="bg-white py-24 rounded-t-[3rem] md:rounded-t-[5rem] text-center shadow-2xl">
        <div className="max-w-5xl mx-auto px-6">
            ...
            <div className="relative aspect-video bg-slate-900 rounded-[2rem] border-[12px] border-slate-100 shadow-2xl overflow-hidden cursor-pointer group" onClick={handleTogglePlay}>
      ...
      {step === 2 && (
          <div className="fixed inset-0 z-[100] bg-slate-950/95 backdrop-blur-md flex items-center justify-center p-4">
              <div className="max-w-xl w-full bg-white rounded-[2rem] p-8 md:p-12 shadow-2xl border-t-[8px] border-blue-600 relative">
                  ...
                  <div className="space-y-8">
                      ...
                      {questions[surveyStep].type === "fields" ? (
                          <div className="grid grid-cols-1 gap-4">
                              ...
                              <button onClick={nextStep} className="mt-4 bg-blue-600 text-white py-4 rounded-xl font-bold hover:bg-blue-700 transition-all">Continue</button>
                          </div>
                      ) : (
                          <div className="grid grid-cols-1 gap-3">
                              {questions[surveyStep].options?.map((opt) => (
                                  <button key={opt} onClick={() => { setFormData({...formData, [questions[surveyStep].name as string]: opt}); nextStep(); }}
                                    className="w-full text-left p-5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-700 hover:bg-blue-600 hover:text-white transition-all">
                                    {opt}
                                  </button>
                              ))}
                          </div>
      ...
      {/* 6. OUTCOME */}
      {step === 3 && (
          <div className="fixed inset-0 z-[100] bg-[#030712] flex items-center justify-center p-4">
              {isQualified ? (
                  <div className="max-w-5xl w-full h-full flex flex-col items-center py-10 space-y-6 overflow-y-auto">
                      ...
                      <div className="w-full bg-white rounded-3xl overflow-hidden shadow-2xl min-h-[650px]">
...
function ChatBubble({ role, text }: { role: 'ai' | 'lead', text: string }) {
    return (
        <div className={`flex ${role === 'lead' ? 'justify-end' : 'justify-start'}`}>
            <div className={`p-4 rounded-2xl text-xs font-medium max-w-[80%] ${
