...
export default function LandingPage() {
  ...
  const questions = [
    ...
    { label: "CONVERSION_EFFICIENCY_METRICS", name: "closingRate", options: ['A) < 15%', 'B) 15% – 25%', 'C) 25% – 40%', 'D) 40%+'] }
  ];
    ...
    <div className="bg-[#020617] text-white min-h-screen font-sans uppercase selection:bg-orange-500 overflow-x-hidden">
      ...
      <section className="max-w-[1440px] mx-auto px-4 md:px-10 pt-6 md:pt-10 pb-24 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center border-b border-white/5">
        <div className="space-y-8 text-left">
           <div className="inline-flex items-center gap-2 bg-indigo-500/10 border border-indigo-500/20 px-3 py-1.5 rounded-full">
             ...
             <span className="text-[9px] md:text-[10px] font-black tracking-widest text-indigo-400 uppercase">Performance-Only Infrastructure</span>
           </div>
           
           <h1 className="text-5xl md:text-8xl font-black tracking-tighter leading-[0.92] text-white uppercase">
             THE ONLY<br/>SALES WORKFORCE<br/>THAT <span className="text-brand-gradient">PAYS FOR ITSELF.</span>
           </h1>
           <div className="h-1.5 w-24 bg-orange-500 shadow-[0_0_20px_#f97316]"></div>
           <p className="text-slate-400 text-lg font-black tracking-tight leading-relaxed max-w-xl uppercase">
             WE INSTALL A 24/7 AI SALES INFRASTRUCTURE INTO YOUR DMS THAT QUALIFIES LEADS AND BOOKS MEETINGS IN &lt; 60 SECONDS. <span className="text-white underline decoration-orange-500 decoration-4 underline-offset-8">0% UPFRONT.</span>
           </p>
        </div>

        {/* MAD IG CHAT MIRROR (VISUAL PROOF) */}
        <div className="relative bg-[#0A0F1E] border border-white/10 rounded-[2.5rem] md:rounded-[3.5rem] shadow-2xl overflow-hidden h-[450px] md:h-[550px] flex flex-col group">
            <div className="p-8 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                <div className="flex items-center gap-3">
                    ...
                    <div><p className="text-sm font-black tracking-tight uppercase">LIVE_MIRROR</p><p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">REALTIME_INGEST: @BLOCKCHAINWIZARD_</p></div>
                </div>
            ...
            <div className="flex-1 p-8 space-y-8 overflow-y-auto custom-scrollbar">
                ...
                <div className="space-y-1"><p className="text-[8px] font-black text-indigo-400 uppercase tracking-widest ml-4">ROLEFLOW_AI</p><div className="bg-white/5 border border-white/10 p-5 rounded-3xl rounded-tl-none text-[11px] font-black text-slate-300 leading-relaxed shadow-inner max-w-[85%] uppercase">AYY APPRECIATE YOU FAM! WE BARELY KEEPING UP WITH DEMAND TBH. YOU READY TO SCALE FR?</div></div>
                <div className="flex justify-end"><div className="bg-indigo-600 p-5 rounded-3xl rounded-tr-none text-[11px] font-black leading-relaxed shadow-xl max-w-[80%] uppercase">100%. TIRED OF THESE LOW QUALITY LEADS VIA ADS.</div></div>
                <div className="space-y-1"><p className="text-[8px] font-black text-indigo-400 uppercase tracking-widest ml-4">ROLEFLOW_AI</p><div className="bg-white/5 border border-white/10 p-5 rounded-3xl rounded-tl-none text-[11px] font-black text-slate-300 leading-relaxed shadow-inner max-w-[85%] uppercase">SAY LESS. I GOT THE FOUNDER'S CALENDAR RIGHT HERE. Let's get you printed.</div></div>
            </div>
        </div>
      </section>
      ...
      <section className="bg-white py-24 md:py-40 rounded-t-[3rem] md:rounded-t-[5rem] text-center">
        <div className="max-w-[1440px] mx-auto px-4 md:px-10 space-y-16">
            <h2 className="text-6xl md:text-9xl font-black text-slate-950 tracking-tighter uppercase leading-none italic">THE_BREAKDOWN.</h2>
            ...
            <button onClick={() => setStep(2)} className="bg-slate-950 text-white px-10 py-6 md:px-24 md:py-10 rounded-[2rem] md:rounded-[3.5rem] font-black text-lg md:text-3xl tracking-[0.3em] shadow-2xl hover:scale-105 active:scale-95 transition-all uppercase flex items-center gap-6 mx-auto group">
                INITIALIZE_ELIGIBILITY_PROTOCOL <ArrowRight className="w-6 h-6 md:w-10 md:h-10 group-hover:translate-x-2 transition-transform" />
            </button>
        </div>
      </section>

      {/* 4. EXPANDED SYSTEM OUTLINE (OUTCOME-DRIVEN) */}
      <section id="system" className="py-24 md:py-40 bg-[#020617] text-center border-t border-white/5">
         <div className="max-w-[1440px] mx-auto px-4 md:px-10 space-y-24">
            <div className="flex flex-col items-center space-y-6">
                <h2 className="text-5xl md:text-9xl font-black tracking-tighter uppercase leading-none text-white">THE UNFAIR<br/>ADVANTAGE.</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10">
               <OutlineCard num="01" title="MARKET_INTELLIGENCE" detail="AUTOMATED EXTRACTION OF HIGH-VOLUME COMPETITORS AND ACTIVE AD CAMPAIGN TARGETS." icon={<Search size={28}/>} />
               <OutlineCard num="02" title="HUNTING_PROTOCOL" detail="HIGH-STATUS ENGAGEMENT PROTOCOLS THAT CONVERT COLD TRAFFIC INTO SALES CONVERSATIONS." icon={<Send size={28}/>} />
               <OutlineCard num="03" title="REVENUE_CAPTURE" detail="24/7 QUALIFICATION & INSTANT CALENDAR DISPATCH. ELIMINATE THE 4-HOUR HUMAN DELAY." icon={<Zap size={28}/>} />
            </div>
         </div>
      </section>
      ...
      {step === 2 && (
          <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-3xl flex items-center justify-center p-4 overflow-y-auto">
              <div className="max-w-2xl w-full bg-white rounded-[3rem] p-8 md:p-16 space-y-12 shadow-2xl relative border-t-8 border-orange-500 my-auto">
                  <div className="absolute top-6 right-8 text-[9px] font-black text-slate-300 tracking-widest uppercase">PROTOCOL_STEP {surveyStep + 1} / {questions.length}</div>
                  ...
                  <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
                      ...
                      {questions[surveyStep].type === "fields" ? (
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              ...
                              <button onClick={nextStep} className="col-span-1 md:col-span-2 mt-4 bg-slate-950 text-white py-5 rounded-[1.5rem] font-black text-xs tracking-widest uppercase shadow-xl active:scale-95 transition-all">CONTINUE_PROTOCOL</button>
                          </div>
                      ) : (
                          <div className="grid grid-cols-1 gap-2 md:gap-3">
                              {questions[surveyStep].options?.map((opt) => (
                                  <button key={opt} onClick={() => { setFormData({...formData, [questions[surveyStep].name as string]: opt}); nextStep(); }}
                                    className="w-full text-left p-5 md:p-8 bg-slate-50 border border-slate-100 rounded-2xl font-black text-[10px] md:text-xs text-black uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all shadow-sm">{opt}</button>
                              ))}
                          </div>
                      )}
                      <button onClick={() => setStep(1)} className="w-full text-[10px] font-black text-slate-300 hover:text-red-500 transition uppercase text-center tracking-widest">ABORT_APPLICATION</button>
                  </div>
              </div>
          </div>
      )}
              ...
              {isQualified ? (
                  <div className="max-w-5xl w-full h-full md:h-auto space-y-10 animate-in zoom-in-95 duration-500">
                      <div className="space-y-4">
                         ...
                         <h2 className="text-4xl md:text-6xl font-black uppercase tracking-tighter text-white">PROTOCOL_ACCEPTED.</h2>
                         <p className="text-slate-400 font-bold text-[10px] tracking-widest uppercase">SELECT_BRIEFING_TIME_BELOW.</p>
                      </div>
                      <div className="bg-white rounded-[2.5rem] md:rounded-[4rem] p-1 md:p-2 shadow-2xl min-h-[600px] md:min-h-[750px] border-4 md:border-8 border-indigo-600/20 overflow-hidden">
                  ...
                  <div className="max-w-md w-full bg-white rounded-[3rem] p-12 space-y-10 text-slate-950 shadow-2xl">
                      ...
                      <h2 className="text-4xl font-black uppercase tracking-tighter text-slate-900 leading-none">ACCESS_DENIED.</h2>
      ...
      <footer className="bg-black py-16 px-10 flex flex-col md:flex-row justify-between items-center gap-12 border-t border-white/5">
         ...
         <div className="flex gap-12 text-[10px] font-black text-slate-600 tracking-widest uppercase">
...
function FormInput({ label, name, val, onChange, type = "text", placeholder }: any) {
    return (
        <div className="space-y-2 text-left">
            <label className="text-[9px] font-black text-slate-400 tracking-widest ml-2 uppercase tracking-widest">{label}</label>
...
function OutlineCard({ num, title, detail, icon }: any) {
    return (
        <div className="bg-white/[0.02] border border-white/5 p-12 rounded-[4.5rem] hover:bg-indigo-600 transition-all group relative overflow-hidden text-center flex flex-col items-center">
            ...
            <h4 className="font-black text-2xl uppercase tracking-tighter text-white mb-4 relative z-10">{title}</h4>
