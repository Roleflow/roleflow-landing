...
import { 
  ...
  BarChart3, Target, Search, Send, Activity, Play, X, Cpu
} from "lucide-react";
  ...
  return (
    <div className="bg-[#020617] text-white min-h-screen font-sans uppercase selection:bg-orange-500 overflow-x-hidden">
      ...
      <section className="max-w-7xl mx-auto px-10 pt-10 pb-24 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center border-b border-white/5">
        ...
        {/* MAD IG CHAT MIRROR */}
        <div className="relative bg-[#0A0F1E] border border-white/10 rounded-[3rem] shadow-2xl overflow-hidden h-[500px] flex flex-col group">
            ...
            <div className="flex-1 p-8 space-y-8 overflow-y-auto">
                ...
                <div className="space-y-1"><p className="text-[8px] font-black text-indigo-400 ml-4 uppercase">ROLEFLOW AI</p><div className="bg-white/5 border border-white/10 p-5 rounded-3xl rounded-tl-none text-[11px] font-black text-slate-300 leading-relaxed shadow-inner max-w-[85%] uppercase">AYY APPRECIATE YOU FAM! WE BARELY KEEPING UP WITH DEMAND TBH. YOU READY TO SCALE FR?</div></div>
            </div>
        </div>
      </section>

      {/* 3. EXTENDED SUB-TEXT */}
      <section className="max-w-7xl mx-auto px-10 py-32 text-center lg:text-left">
      ...
      {/* 4. THE VSL SECTION - NATIVE PLAYER */}
      <section className="bg-white py-32 rounded-t-[5rem] text-center space-y-20">
        <div className="relative group max-w-5xl mx-auto px-6">
            ...
            <div className="relative aspect-video bg-black rounded-[3rem] border border-slate-200 shadow-2xl overflow-hidden cursor-pointer group" onClick={handleTogglePlay}>
      ...
      <section id="system" className="py-40 bg-[#020617] text-center relative border-t border-white/5">
         <div className="max-w-7xl mx-auto px-10 space-y-32">
            <div className="space-y-6 flex flex-col items-center">
                ...
                <div className="h-2 w-48 bg-orange-500 shadow-[0_0_20px_#f97316]"></div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
               <OutlineCard num="01" title="ICP SCRAPER" detail="EXHAUSTIVE EXTRACTION OF ACCOUNTS RUNNING ACTIVE PAID MEDIA." icon={<Search size={32}/>} />
               <OutlineCard num="02" title="OUTREACH AGENT" detail="HIGH-STATUS ENGAGEMENT PROTOCOLS TO INITIATE SALES THREADS." icon={<Send size={32}/>} />
               <OutlineCard num="03" title="INBOUND SETTER" detail="24/7 QUALIFICATION & INSTANT CALENDAR DISPATCH LOGIC." icon={<Zap size={32}/>} />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16 pt-20 border-t border-white/5 max-w-5xl mx-auto text-left">
                <div className="flex items-center gap-6">
                    ...
                    <div><h5 className="font-black text-xl text-white">4-PILLAR PROTOCOL</h5><p className="text-[10px] text-slate-500 tracking-widest font-black uppercase">PERSONA • KNOWLEDGE • LOGIC • PROPERTIES</p></div>
                </div>
                <div className="flex items-center gap-6">
                    <BarChart3 size={32} className="text-orange-500 shrink-0" />
                    <div><h5 className="font-black text-xl text-white">DIRECT ATTRIBUTION</h5><p className="text-[10px] text-slate-500 tracking-widest font-black uppercase">REAL-TIME REVENUE TRACKING VIA CRM</p></div>
                </div>
            </div>
         </div>
      </section>

      {/* 6. INTERACTIVE SURVEY (ONE BLOCK AT A TIME) */}
      {step === 2 && (
          <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-3xl flex items-center justify-center p-6">
              <div className="max-w-2xl w-full bg-white rounded-[4rem] p-10 md:p-16 space-y-12 shadow-2xl relative border-t-8 border-indigo-600">
                  ...
                  <div className="space-y-10 animate-in slide-in-from-bottom-4 duration-500">
                      ...
                      {questions[surveyStep].type === "fields" ? (
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                              ...
                              <button onClick={nextStep} className="col-span-1 md:col-span-2 mt-4 bg-indigo-600 text-white py-6 rounded-3xl font-black text-sm uppercase shadow-xl hover:bg-indigo-700 transition-all">CONTINUE PROTOCOL</button>
                          </div>
                      ) : (
                          <div className="grid grid-cols-1 gap-4">
                              {questions[surveyStep].options?.map((opt) => (
                                  <button key={opt} onClick={() => { setFormData({...formData, [questions[surveyStep].name as string]: opt}); nextStep(); }}
                                    className="w-full text-left p-8 bg-slate-50 border border-slate-100 rounded-[2rem] font-black text-sm text-black uppercase tracking-widest hover:bg-slate-950 hover:text-white transition-all shadow-sm">{opt}</button>
                              ))}
                          </div>
                      )}
                      <button onClick={() => {setStep(1); setSurveyStep(0);}} className="w-full text-[10px] font-black text-slate-300 hover:text-red-500 transition uppercase tracking-widest text-center">Abort Application</button>
                  </div>
              </div>
          </div>
      )}
      ...
      <footer className="bg-black py-16 px-10 flex flex-col md:flex-row justify-between items-center gap-12 border-t border-white/5">
         ...
         <p className="text-[10px] font-black text-slate-800 tracking-widest uppercase">© 2026 ROLEFLOW.IO. ALL RIGHTS RESERVED.</p>
      </footer>
    </div>
  );
}
...
function FormInput({ label, name, placeholder, onChange, type = "text", val }: any) {
    return (
        <div className="space-y-2 text-left">
            ...
            className="w-full bg-slate-50 border border-slate-100 p-6 rounded-[1.5rem] text-sm font-black text-black focus:ring-4 focus:ring-indigo-500/10 outline-none uppercase transition-all placeholder:text-slate-300 shadow-inner" placeholder={placeholder} />
        </div>
    )
}
