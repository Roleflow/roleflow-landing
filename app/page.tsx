...
import { 
  ...
  Play, X, Cpu, Search, BarChart3, Lock, MessageSquare, Star, TrendingUp, Calculator
} from "lucide-react";
...
export default function LandingPage() {
  ...
  // --- CALCULATOR STATE ---
  const [calc, setCalc] = useState({ dms: 400, rate: 5, price: 5000 });
  
  // High Ticket Math Logic: 
  // Current: DMs * Booking Rate * 20% Closing Rate * Price
  // AI Projected: DMs * (Booking Rate + 10% gain) * 20% Closing Rate * Price
  const currentRev = Math.round(calc.dms * (calc.rate / 100) * 0.20 * calc.price);
  const aiRev = Math.round(calc.dms * ((calc.rate + 10) / 100) * 0.20 * calc.price);
  const gap = aiRev - currentRev;
      ...
      <section className="max-w-7xl mx-auto px-6 pt-12 pb-20 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="space-y-8">
           ...
           <p className="text-slate-400 text-lg md:text-xl font-bold leading-relaxed max-w-xl text-xs tracking-widest">
             ROLEFLOW INSTALLS A PROPRIETARY AI SALES WORKFORCE THAT QUALIFIES LEADS AND BOOKS HIGH-TICKET MEETINGS IN LESS THAN 60 SECONDS. <span className="text-white border-b-2 border-blue-600 font-black">24/7 COVERAGE. ZERO HEADCOUNT.</span>
           </p>
      ...
      <section className="py-32 bg-white/5 border-y border-white/5">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-20">
            <p className="text-blue-500 font-bold tracking-[0.3em] text-[10px] mb-4">THE MATHEMATICS OF AI</p>
            <h2 className="text-4xl md:text-6xl font-extrabold text-white tracking-tight leading-none uppercase">THE REVENUE GAP CALCULATOR</h2>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            {/* INPUTS */}
            <div className="bg-white/5 border border-white/10 p-8 md:p-12 rounded-[2.5rem] space-y-10 shadow-inner">
              <div className="space-y-4">
                <div className="flex justify-between uppercase text-[10px] font-bold tracking-widest text-slate-400">
                  <span>Monthly Inbound DMs</span>
                  <span className="text-blue-500 font-black">{calc.dms}</span>
                </div>
                <input type="range" min="50" max="2000" step="50" value={calc.dms} onChange={(e) => setCalc({...calc, dms: parseInt(e.target.value)})} className="w-full h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-blue-600" />
              </div>
              <div className="space-y-4">
                <div className="flex justify-between uppercase text-[10px] font-bold tracking-widest text-slate-400">
                  <span>Current Booking Rate (%)</span>
                  <span className="text-blue-500 font-black">{calc.rate}%</span>
                </div>
                <input type="range" min="1" max="20" step="1" value={calc.rate} onChange={(e) => setCalc({...calc, rate: parseInt(e.target.value)})} className="w-full h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-blue-600" />
              </div>
              <div className="space-y-4">
                <div className="flex justify-between uppercase text-[10px] font-bold tracking-widest text-slate-400">
                  <span>Average Offer Price ($)</span>
                  <span className="text-blue-500 font-black">${calc.price.toLocaleString()}</span>
                </div>
                <input type="range" min="1000" max="20000" step="500" value={calc.price} onChange={(e) => setCalc({...calc, price: parseInt(e.target.value)})} className="w-full h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-blue-600" />
              </div>
            </div>

            {/* RESULTS */}
            <div className="bg-blue-600 p-8 md:p-12 rounded-[2.5rem] shadow-2xl shadow-blue-600/20 space-y-8 relative overflow-hidden">
              <div className="bg-white/10 p-8 rounded-2xl border border-white/20 text-center relative z-10">
                <p className="text-[10px] font-bold tracking-[0.3em] uppercase mb-2 text-white opacity-70">The Hidden Revenue Gap</p>
                <h4 className="text-5xl md:text-7xl font-black text-white uppercase tracking-tighter">
                  +${gap.toLocaleString()}
                </h4>
                <p className="text-[10px] font-bold tracking-widest uppercase mt-4 text-blue-100 border-b border-blue-400/30 inline-block pb-1">Hidden money recovered by instant AI response</p>
              </div>
              <div className="flex justify-between items-center text-white/80 px-4 relative z-10">
                <div className="text-center">
                  <p className="text-[9px] font-bold uppercase tracking-widest mb-1 opacity-60">Current Rev</p>
                  <p className="text-xl font-bold tracking-tighter">${currentRev.toLocaleString()}</p>
                </div>
                <TrendingUp size={24} className="opacity-40" />
                <div className="text-center">
                  <p className="text-[9px] font-bold uppercase tracking-widest mb-1 opacity-60">Projected Rev</p>
                  <p className="text-xl font-bold tracking-tighter">${aiRev.toLocaleString()}</p>
                </div>
              </div>
              <BarChart3 className="absolute -bottom-10 -right-10 w-64 h-64 opacity-5 text-white" />
            </div>
          </div>
        </div>
      </section>

      {/* 5. VSL & REVIEWS SECTION */}
      <section className="bg-white py-24 rounded-t-[3rem] md:rounded-t-[5rem] text-center shadow-2xl">
        <div className="max-w-6xl mx-auto px-6">
            <div className="mb-12">
               ...
               <h2 className="text-4xl md:text-6xl font-extrabold text-slate-900 tracking-tight uppercase leading-none">TRUSTED BY HIGH-TICKET FOUNDERS</h2>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left mb-20">
              {testimonials.map((t, i) => (
                <div key={i} className="bg-white border border-slate-200 p-8 rounded-[2.5rem] space-y-6 shadow-sm flex flex-col justify-between">
                  <div className="space-y-4">
                    ...
                    <p className="text-slate-600 font-bold leading-relaxed uppercase text-[11px] tracking-wide border-l-2 border-blue-500 pl-4">
                        "{t.text}"
                    </p>
                  </div>
                  <div className="pt-6 border-t border-slate-100 flex justify-between items-end">
                    <div>
                      <p className="font-extrabold text-slate-900 uppercase text-xs tracking-tighter leading-none mb-1">{t.company}</p>
                      <p className="text-[9px] text-slate-400 font-bold tracking-[0.2em] uppercase leading-none">{t.industry}</p>
                    </div>
                    <p className="text-blue-600 font-black text-xs uppercase tracking-tighter whitespace-nowrap leading-none">{t.result}</p>
                  </div>
                </div>
              ))}
            </div>

            <button onClick={() => setStep(2)} className="bg-slate-950 text-white px-12 py-7 rounded-2xl font-bold text-xl hover:bg-blue-600 transition-all flex items-center gap-4 mx-auto uppercase tracking-tighter shadow-2xl">
              ...
              <div className="max-w-xl w-full bg-white rounded-[2.5rem] p-8 md:p-14 shadow-2xl border-t-[8px] border-blue-600 relative text-slate-950">
                  ...
                  <div className="space-y-10">
                      <div className="space-y-2 text-left">
                          ...
                          <h2 className="text-3xl font-extrabold tracking-tight leading-none uppercase">{questions[surveyStep].label}</h2>
                      </div>
                          ...
                          <div className="grid grid-cols-1 gap-3">
                              {questions[surveyStep].options?.map((opt) => (
                                  <button key={opt} onClick={() => { setFormData({...formData, [questions[surveyStep].name as string]: opt}); nextStep(); }}
                                    className="w-full text-left p-6 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-slate-800 hover:bg-blue-600 hover:text-white transition-all uppercase text-sm tracking-widest">
                                    {opt}
                                  </button>
                              ))}
                          </div>
              ...
              {isQualified ? (
                  <div className="max-w-5xl w-full h-full flex flex-col items-center py-12 space-y-8">
                      ...
                      <div className="text-center space-y-2">
                        <h2 className="text-4xl md:text-5xl font-black text-white uppercase tracking-tighter leading-none uppercase">APPLICATION APPROVED</h2>
                  ...
                  <div className="max-w-md w-full bg-white rounded-[3rem] p-16 text-center space-y-8 text-slate-950 uppercase">
                        <Lock size={64} className="mx-auto text-slate-200" />
                        <h2 className="text-4xl font-extrabold tracking-tight leading-none">NOT A FIT</h2>
                        <p className="text-slate-500 font-bold text-[10px] tracking-widest leading-relaxed">YOUR CURRENT BUSINESS METRICS DO NOT QUALIFY FOR OUR AI INFRASTRUCTURE. PLEASE RETURN WHEN YOU ARE GENERATING AT LEAST $15K/MONTH IN REVENUE.</p>
                        <button onClick={() => setStep(1)} className="text-blue-600 font-bold text-xs border-b-2 border-blue-600 pb-1">RETURN HOME</button>
                  </div>
              )}
          </div>
      )}
